...
<link rel="stylesheet"
    ...
    <script>
        function fun() {
            ...
            /*第一种请求方式*/
            $.post("/product/getAllajax.do",{page:1,pageSize:4},function (data){
                $("#btn1").empty();
                //循环data里面的List
                $.each(data,function (i,product) {
                    $('#btn1').append(
                        '<tr><td><input type="checkbox" name="ids" value="'+product.pid+'"></td>'+
						'<td>'+product.pid+'</td>' +
                        ...
                        '<td>'+product.productStatus+'</td></tr>'
					)
                })
            })
    ...
    <script>
        function del() {
                ...
                if ($("input[type='checkbox']:checked").length > 0) {
                    ...
                    //Post请求并把数组传过去
                    $.post("/product/delete.do?id="+checkID,{},function (data) {
                        //如果后台执行完并返回数据则提示并跳转
                        if (data!=null){
                            alert("删除成功")
                            window.location.href="/product/getAll.do?page=1&pageSize=4";
                            //如果后台出现异常则提示删除失败
                        }else{
                            alert("删除失败")
                        }
			...
			<section class="content">
				...
				<div class="box box-primary">
					...
					<div class="box-footer">
						...
						<div class="box-tools pull-right">
							<ul class="pagination">
								<li><a href="/product/getAll.do?page=1&pageSize=${pageInfo.pageSize}" aria-label="Previous">首页</a></li>
								<li><a href="/product/getAll.do?page=${pageInfo.pageNum-1}&pageSize=${pageInfo.pageSize}">上一页</a></li>
								<c:forEach begin="1" end="${pageInfo.pages}" var="i">
									<li><a href="/product/getAll.do?page=${i}&pageSize=${pageInfo.pageSize}">${i}</a></li>
								</c:forEach>
								<li><a href="/product/getAll.do?page=${pageInfo.pageNum+1}&pageSize=${pageInfo.pageSize}">下一页</a></li>
								<li><a href="/product/getAll.do?page=${pageInfo.pages}&pageSize=${pageInfo.pageSize}" aria-label="Next">尾页</a></li>
							</ul>
						</div>
